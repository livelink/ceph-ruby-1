module CephRuby
  # Pool Helper MEthods
  module PoolHelper
    def self.by_id(cluster, id, &block)
      Pool.new(cluster, cluster.pool_name_by_id(id), &block)
    end

    def create_with_all(auid, rule_id)
      ret = Lib::Rados.rados_pool_create_with_all(clusterr_handle, name,
                                                  auid, rule_id)
    ...
    def create_with_rule(rule_id)
      ret = Lib::Rados.rados_pool_create_with_crush_rule(cluster_handle, name,
                                                         rule_id)
